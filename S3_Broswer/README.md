...
- Downloads skip files already present locally when size + MD5 match the S3 ETag (RunPod ETag = content MD5)
...
- App settings (`config.json`) live next to the EXE (portable — runs from a thumb drive); never in `%APPDATA%`
