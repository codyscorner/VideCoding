...
### v2.4.3
- **Chooser columns are character budgets now, and there is a row number.** v2.4.1/2 measured each column against its current text and still elided ("RTX PRO 6000 Black…", "2h 29…", "$2.…"), because the style's own cell margins weren't in the sum. Each column now has a fixed budget in characters — # 3, Pod 25, GPU 25, Up 10, $/hr 10, Spent 10, note 20 — converted to pixels in the real font plus the cell padding and margins, so a normal value never clips and a longer one elides with the full text in its tooltip. The pod name no longer stretches; the note column absorbs any spare width. Rows are numbered 1, 2, 3… in a first column.

### v2.4.2
- **White bar at the left of every cell on the chooser's selected row, gone.** The native Windows widget style paints its own rounded highlight frame per cell on a selected row, whatever the stylesheet says, and the light edge of that frame showed at every cell boundary. The table now uses the Fusion style, which paints exactly what the stylesheet describes, and the per-item focus frame is switched off too.

### v2.4.1
- **Pods-already-running chooser tidied.** The first live showing had the GPU column stretched across most of the width with the pod name, uptime and spend crammed and clipped at the edges. Now the pod name — whatever you called it in the console, so it can be long — takes all the spare width, and every other column is sized to its own widest text in the real font plus padding. Numbers are right-aligned, cells padded, no grid lines, one line per pod (an over-long name elides rather than wraps), rows uniform, the table exactly as tall as its rows (up to six), the selected row in the accent colour, and the dialog between 900 and 1300 px wide.
- The "didn't shut down cleanly" line on the first v2.4.0 launch was expected: the session file it read had been written by v2.3.0, before the left-running flag existed. From now on a pod left up on purpose gets the "You left … running" wording.

### v2.4.0
- **Launch shows what's running and asks, instead of silently grabbing a pod.** With the Chain Automator on one pod and Video Creator wanting another, the old launch check would quietly point this app at whichever pod was up. Now, when any pod on the account is running (and the app is in RunPod mode, or the running pod is the one this app had last time), a **RunPod — pods already running** dialog lists each one with live status — name, GPU, uptime, $/hr, spend so far — marking *this app's pod* and the *current connection*. Four choices:
  - **Use selected** (default; double-click a row does the same) connects to it. The pod this app started last time stays app-owned, so the stop-or-leave question comes back on exit; any other pod is treated as someone else's and never auto-stopped.
  - **Start another pod** leaves every running pod to whatever is using it and walks the usual chain for a stopped one. The running pods are excluded from that chain — and from every Keep-Trying sweep and header-button start for the rest of the session — so "start another" can't come back with the pod you said to leave alone.
  - **Stop selected** stops it on the spot (this app's session file is cleared if it was that pod), then the chooser comes back for whatever is still up, or the plain "start a pod?" prompt if nothing is.
  - **Skip** leaves everything alone; the header button starts a pod later.
  - In Local mode a foreign running pod is only logged, no dialog. When nothing is running, the "Start a RunPod pod?" prompt is unchanged.
- **Honest wording for a pod you chose to leave running.** Answering *No* to "Stop the pod?" on exit now records that choice in the session file, so the next launch opens with "You left tart_RTX_6000 4 running when you last quit." instead of claiming the app "didn't shut down cleanly". A real crash still says so.
- **Launch waits for a pod that is still reporting in.** RunPod flips a pod to RUNNING a few seconds before the container reports its GPU. A pod started moments before the app launched read as unhealthy, was skipped, and the app then offered to start *another* one. The launch check now gives such a pod the same 60 s grace the pod starter got in v2.1.1 (a small cancellable "waiting for its GPU to report" dialog), and only then decides. The zero-GPU trap (RUNNING with an empty GPU list) is still never offered.
- Verified headless against a scripted RunPod with 12 checks: left-running vs crash wording; Use / Start another / Stop / Skip; the stopped pod not being re-offered; a pending pod reporting in and being offered; one that never reports falling through to the start prompt; the zero-GPU pod; Local mode; a vanished session pod; and the start chain neither reusing nor restarting a skipped pod. Dialog layout checked with an offscreen grab at 900 px.

### v2.3.0
- **Average sampler step time, recorded per video.** The run time from v2.2.0 says how long the whole thing took, but most of that is denoising — the rest is model loading, VAE decode and saving — so the part worth comparing is the sampler. The worker now times the gap between consecutive progress events from the sampler nodes and stores three more numbers on the run's history entry: `sampler_steps` (steps run, summed over samplers, so a WAN hi/lo split counts both halves), `step_seconds` (the average per step) and `sampler_seconds` (steps × average — about how long the run spent denoising).
  - **Library detail pane:** the made-from line reads `⏱ generated in 12m 34s (render 11m 50s · 48 steps @ 3.2 s/step)`; the settings dump gains a `Sampler: 48 steps @ 3.2 s/step — about 2m 34s denoising` line. **History window** rows carry the same `⏱` text. The run log's closing line says `Done in 12m 34s (render 11m 50s, 48 steps @ 3.21 s/step)`.
  - Only sampler nodes count (`KSampler`, `KSamplerAdvanced`, `SamplerCustomAdvanced`, the WAN wrappers — any class with "sampler" in its name). ComfyUI now tags each progress event with the node that sent it, so progress from a tiled VAE decode or an API node polling its job is ignored rather than diluting the average. The first event of each sampler pass has no gap to measure (the time before it holds the model load), so an N-step pass is averaged over N−1 gaps; the step count is still N.
  - A run whose workflow has no sampler (the MiniMax H3 API node does its own denoising) records the run and render times as before and simply has no step figure. Entries from v2.2.0 and earlier show what they have.
- Verified offline: a 4-step pass behind a 20 s model load averages exactly the step time; a WAN hi/lo pair with tiled-decode progress between them counts 5 steps and ignores the tiles; an API-node-only run times nothing; a progress bar that resets starts a fresh gap; storage and both display strings round-trip through the history sidecar; v2.2.0 and older entries render unchanged.

### v2.2.0
- **Every video now records how long it took to make.** When a run finishes, the worker hands back two numbers with the output files and they are written onto the run's prompt-history entry next to the result names: `run_seconds` (the whole thing, from Create to the file on disk — upload, queue wait, render, download, stitch) and `render_seconds` (just the ComfyUI part, queued to finished). The gap between them is what the RunPod proxy round-trips cost.
  - **Library detail pane:** the "made from" line under the file info reads `MiniMax_Vid_Creator/MiniMax_Fighting.json  ·  2026-09-12 14:03:11  ·  ⏱ generated in 12m 34s (render 11m 50s)`, and the settings dump below it gains a `Generated in:` line after `Saved:`.
  - **History window:** each row ends with `⏱ 12m 34s (render 11m 50s)`.
  - The render part is left off when it isn't known or is the whole run anyway (a local server with nothing to upload). Entries made before this version have no timing and show nothing new — nothing to migrate. The Chain Automator shares these files and ignores keys it doesn't know.
  - The run log's closing line now says `Done in 12m 34s (render 11m 50s)`.
- `build_exe.py --no-deploy` builds the EXE and leaves it in `dist/` without touching the Apps folder — for a build made while the deployed app is busy running.
- Verified offline: timing written and read back through the history sidecar, the Library lookup by result name, both display strings, sub-minute and equal-render formatting, and entries with no timing (older ones, and runs that report none) left untouched.

### v2.1.1
- **Pod starter no longer rejects a good pod as "started with no GPU attached".** RunPod flips a pod's status to RUNNING a few seconds *before* the container reports in, and until it does the record's `runtime` block (uptime, ports, the attached-GPU list) is null. The health check read a missing block as "no GPU" and `wait_running` judged it on the very first RUNNING reading — so at 09:12 on 2026-09-12 the A100 pod was read RUNNING and stopped in the same second while its card was sitting right there in the console. Now `runpod_api.gpu_state()` tells four cases apart — `ok`, `pending` (RUNNING, no report yet), `none` (the zero-GPU trap: report present, GPU list empty, or `gpu.count` 0) and `off` — and `wait_running` gives a pending pod `RUNTIME_GRACE` (60 s from the first RUNNING reading) to report before giving up. The genuine zero-GPU trap is still rejected on the spot, and a pod that never reports is still stopped so it can't bill unnoticed. The 409 branch of `start_pod` and the header's periodic poll make the same distinction, so a pending pod is neither skipped nor "forgotten".
- Verified offline with a 16-check simulation of the RUNNING-before-runtime sequence (the 09:12 case, never-reports, zero-GPU, `gpu.count` 0, the EXITED grace chained with the new one, and both 409 outcomes), and live against the account by starting a stopped pod with the new wait and stopping it the moment its GPU was confirmed.

### v2.1.0
- **New ✍ Text → Video tab** — third tab, between Video → Extend and the Library. There is nothing to pick from, so there is no thumbnail browser: the run panel (and its prompt editor) takes the whole width. Pick a prompt-only workflow, write the prompt, **Create Video**. Everything the other tabs have works here unchanged — type-to-filter workflow dropdown and ⧉ Clone, ⤢ Expand, the 🪄 AI Rewriter (in T2VA mode, since there is no reference image), LoRA rows, seed / steps / megapixels / length, the Turbo toggle, prompt history with settings, the shared run queue with its repeat guard, the Results list and player.
  - **Output name box** where the other tabs show the selected source. A text-to-video clip has no source to be named after, so type one: `rain_walk` saves as `rain_walk_<workflow>_<timestamp>.mp4`; blank saves as `T2V_<workflow>_<timestamp>.mp4`. Remembered between runs, and part of the repeat check, so the same prompt under two names is two runs.
  - **Only prompt-only workflows run here.** One with a `LoadImage` / `LoadVideo` / folder-loader node is refused — the status line says why and Create stays disabled — because it would run against whatever file name is baked into that node (usually one the server doesn't have) and wouldn't be text-to-video anyway. The worker checks again, so nothing slips through the queue.
  - **Library → 🔁 Reuse Settings knows the tab.** Runs from here write `text_to_video: true` into their history entry, and the embedded-metadata fallback flags a graph with no loader node the same way, so a retry lands on the Text tab with the workflow, prompt and settings restored and saves back into the video's own folder. History rows and the detail pane say "text → video".
  - The queue view lists a text run as `(prompt only → <name>)` in the Source column with no start frame; the "next up" label under the buttons uses the same wording.
  - Config: `text_workflow`, `text_output_name`, `panel_split_text`.
- **Sample workflow** `MiniMax_Vid_Creator/MiniMax_T2V.json` added next to the other MiniMax templates in the Workflow_API folder (not part of this repo), derived from `MiniMax_Sandbox.json` in the same folder: the LoadImage → ImageScaleToTotalPixels → GetImageSize chain is dropped, `MiniMaxH3ImageToVideo`'s optional `first_frame` is left unwired (which is exactly what makes it T2VA), and its width/height now come from the ResolutionSelector that was already in the graph — so the Megapixels box and the selector's aspect ratio decide the frame size. Turbo LoRA, duration and the VHS h265 output are untouched.
- Verified headless against a mock ComfyUI server: 39 checks — workflow gating both ways, two runs (named and blank output name), the graph sent has no loader and no `first_frame`, history flags and result names, queue view with a source-less request, the repeat guard, Reuse Settings routing for text and image entries, the embedded-metadata fallback for both graph kinds, and an offscreen grab of the tab at 1640×960.
...
- **Pods are now tried by GPU model first: every RTX 6000 before any A100.** The chain used to walk a hand-dragged list of pod *ids*, which meant the preference died the moment a pod was rebuilt — a brand new RTX 6000 got appended to the bottom of the list, behind both A100s, because the order had never seen its id. Settings gains a **GPU priority** list above the pod list: rank the card models once and every pod on the better card is tried before any pod on the lesser one, no matter when it was created.
  ...
  - **▲▼ on a pod moves it within its own card group.** Crossing a boundary is refused with a note pointing at the GPU list, rather than being silently undone on the next redraw.
  - **A running pod still leads its own group** — using it costs nothing to start and nothing to wait for — but running-ness no longer outranks the card. A running A100 does not jump ahead of a stopped RTX 6000; the pre-pass that adopts an already-warm pod walks this same order.
  - **Neither list is a whitelist.** A card model you haven't ranked sorts last, a pod the per-pod order has never seen sorts last within its group, and both cases are logged — but every pod on the account is always tried. A preference must never make hardware invisible, which was the v1.7.3 bug in a new guise.
  - The order is stored as `runpod_gpu_order`. Existing configs have none, so the chain behaves exactly as it did in v1.7.5 until you set one: open **Settings → RunPod**, drag your preferred card to the top, and press OK.
- **Settings is now tabbed — Server / Folders / RunPod / Prompts.** The two-column form had run out of room: adding the GPU list pushed it to 1171px tall, which does not fit a 1080p screen, and every row was squeezed into half the dialog's width. Each tab is only as tall as it needs to be and gets the full width, so the dialog is **1120×797** with 200px to spare.
  - The pod list shows **all six pods at once** with their name, GPU, datacenter and status untruncated — previously three rows and a horizontal scrollbar.
  - Folder paths are readable end to end instead of being cut off mid-path.
- **The pod list fills itself in when you open Settings.** It used to show bare pod ids like `1vp35krguwhra1` until you pressed Refresh — so you could not tell which row was which card, which made ordering by GPU impossible without a fetch first. The fetch now runs automatically on open, on a thread, so opening Settings never blocks on RunPod being reachable. With no API key it says so instead of showing ids.
