"""The MiniMax-H3 video prompt writing guides, embedded verbatim so the local
rewriter (prompt_rewriter.py) can hand them to any LLM as a system prompt
without a network fetch. Source: the h3-prompt-writing skill's reference docs,
which match MiniMax's own writing guide.
"""

GUIDE_BASE = r"""# Video Prompt Writing Guide (T2VA / I2VA / FL2VA / L2VA)
...
GUIDE_REF = r"""# Full-Reference Mode Rewrite Output Format Guide
...
When an `<Audio N>` explicitly corresponds to a target speaker, reuse that speaker's global ID in the definition: write `<Subject N> (Sx)` when the speaker maps to a defined subject, or use a stable voice description followed by `(Sx)` otherwise. The ID comes from the target video's global speaker order and is not independently assigned or renumbered in the audio definition:
